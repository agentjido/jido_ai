defmodule Jido.AI.Accuracy.Generators.LLMGenerator do
  ...
  - `:model` - Model spec or alias (default: `#{@default_model}`)
  - `:num_candidates` - Number of candidates (default: `#{@default_num_candidates}`)
  - `:temperature_range` - Temperature range `{min, max}` (default: `{0.0, 1.0}`)
  - `:timeout` - Timeout in ms (default: `#{@default_timeout}`)
  - `:max_concurrency` - Max parallel requests (default: `#{@default_max_concurrency}`)
  - `:system_prompt` - Optional system prompt
  ...
  def new(opts) when is_list(opts) do
    ...
    resolved_model =
      case model do
        atom when is_atom(atom) -> Config.resolve_model(atom)
    ...
    with true <- valid_temperature_range?(temperature_range),
         {min_temp, max_temp} <- temperature_range,
         true <- min_temp >= 0 and max_temp <= 2 and min_temp <= max_temp do
      {:ok,
       %__MODULE__{
      ...
      _ -> {:error, :invalid_temperature_range}
    end
  end
  ...
  defp valid_temperature_range?({min, max}) when is_number(min) and is_number(max),
    do: true

  defp valid_temperature_range?(_), do: false
