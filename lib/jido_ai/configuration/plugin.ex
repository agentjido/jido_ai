defmodule Jido.AI.Configuration.Plugin do
  ...
  def native_binding(admission) do
    ...
    if agent, do: Jido.AI.Orchestration.Binding.request(agent, admission.signal)
  end
  ...
  defp fallback_agent(context, module, opts) do
      ...
      case Map.to_list(profiles) do
        [{id, _profile}] ->
          if String.ends_with?(context.signal.type, ".query") or
            ...
            target = Jido.AI.Orchestration.Start
    ...
    plugins =
      if Map.has_key?(context.agent_state, Jido.AI.Thread.Control.key()),
        do: plugins ++ [{Jido.AI.Thread.Control.Plugin, [profiles: histories]}],
        else: plugins
