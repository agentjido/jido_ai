defmodule Jido.AI.Observe do
  ...
  defdelegate sanitize_telemetry_metadata(metadata, opts \\ []),
    to: Sanitize,
  ...
  defdelegate sanitize_transport_payload(payload, opts \\ []),
    to: Sanitize,
    as: :transport_payload

  defp emit_enabled?(obs_cfg, opts) do
    Map.get(obs_cfg, :emit_telemetry?, true) and
      feature_gate_enabled?(obs_cfg, Keyword.get(opts, :feature_gate))
  end
  ...
  defp valid_event?([:jido, :ai, :request, event])
       when event in [:start, :complete, :failed, :rejected, :cancelled],
       do: true
