defmodule Jido.AI.Effects do
  @moduledoc """
  Facade for tool effect policy and complete candidate-state assembly.

  Return `state(next_state)` from a tool to propose a complete next Agent state.
  Use core typed Directives for work that must follow commit. The Agent and
  reasoning policies are intersected; the reasoning policy can only narrow.
  Direct I/O performed by the Action remains outside the state transaction.

  `apply_result/3` builds an immutable Agent value and returns pending Directives.
  It does not commit or dispatch. A rejected proposal retains the original Agent,
  returns no Directives, and puts the error in the returned canonical result.
  """

  alias Jido.AI.Effects.{Applier, Policy, State}
  alias Jido.Agent

  @doc "Proposes a complete next state. Core commit remains the state owner."
  def state(state) when is_map(state) and not is_struct(state), do: %State{state: state}
