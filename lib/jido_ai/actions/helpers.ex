defmodule Jido.AI.Actions.Helpers do
  ...
  * `params` - Map containing :max_tokens, :temperature, :timeout keys
  ...
  Delegates to `Jido.AI.Model.Response.extract_text/1` which handles
  ...
  * `response` - LLM response map
  ...
  ## Examples
      ...
      "Hi"
  """
  defdelegate extract_text(response), to: Response
  ...
  * `{:error, reason}` - Validation failed
  ...
  def validate_and_sanitize_input(params, opts \\ []) do
    ...
    max_system_prompt_length =
      Keyword.get(opts, :max_system_prompt_length, Validation.max_prompt_length())

    with {:ok, _prompt} <-
           validate_prompt_if_required(params[:prompt], required_prompt, max_prompt_length),
  ...
  defp validate_prompt_if_required(_, true, _max_length), do: {:error, :invalid_string_type}
  ...
  * `result` - The result tuple to format
  ...
  def emit_llm_error(obs_cfg, start_time, base_metadata, reason) when is_map(base_metadata) do
    ...
    measurements =
      if is_map(base_metadata[:usage]),
        do: Map.merge(measurements, token_measurements(base_metadata.usage)),
        else: measurements
