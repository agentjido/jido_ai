defmodule Jido.AI.Observe.Sanitize do
  ...
  def sensitive(%_{} = payload), do: payload |> Map.from_struct() |> sensitive()
  ...
  def sensitive(payload) when is_list(payload) do
    if proper_list?(payload),
      do: Enum.map(payload, &sensitive/1),
      else: inspect_limited(payload, 512)
  end
  ...
  defp sanitize_opts(defaults, opts) do
    Enum.reduce(opts, defaults, fn
      {key, value}, acc when is_map_key(acc, key) and is_integer(value) and value > 0 ->
  ...
  defp sanitize_value(value, _profile, _opts, _depth)
  ...
  defp summary_base(value) when is_map(value),
    do: %{type: :map, size: map_size(value), keys: summary_keys(value)}
  ...
  defp maybe_add_summary_details(summary, {:ok, payload, effects}, profile, opts)
  ...
  defp maybe_add_summary_details(summary, {:error, error, effects}, profile, opts)
       when is_list(effects) do
    Map.merge(summary, %{
      status: :error,
  ...
  defp maybe_add_summary_details(summary, value, _profile, opts) when is_binary(value) do
    Map.put(
      summary,
      :truncated?,
      String.valid?(value) and String.length(value) > opts.max_string_chars
    )
  end
  ...
  defp summarize_error(error, _profile, opts) when is_exception(error) do
    %{
      type: module_label(error.__struct__),
      message: sanitize_binary(Exception.message(error), opts.max_string_chars)
    }
  end

  defp summarize_error(%{} = error, _profile, opts) do
    %{}
    ...
    |> maybe_put_summary_field(
      :retryable?,
      Map.get(error, :retryable?, Map.get(error, "retryable?"))
    )
    |> case do
      empty when empty == %{} ->
        summary_base(Map.drop(error, [:details, "details"]))
  ...
  defp sanitize_summary_message(message, opts),
    do: inspect_limited(message, opts.max_inspect_chars)
  ...
  defp sanitize_key(key) when is_binary(key),
    do: sanitize_binary(key, @transport_defaults.max_string_chars)
  ...
  defp telemetry_payload_summary_key?(key),
    do: MapSet.member?(@telemetry_payload_summary_keys, key)
