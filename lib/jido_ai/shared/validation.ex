defmodule Jido.AI.Validation do
  ...
  def validate_callback(callback) when is_function(callback),
    do: {:error, :invalid_callback_arity}
  ...
  The default wrapper uses core Exec and needs no named task supervisor.
  An explicit `task_supervisor` option keeps caller-owned Task execution.
  """
  ...
  def validate_and_wrap_callback(callback, opts) when is_function(callback) do
    with :ok <- validate_callback(callback) do
      ...
      case Keyword.fetch(opts, :task_supervisor) do
        :error ->
          {:ok, wrap_with_exec(callback, timeout)}

        {:ok, task_supervisor} ->
          with {:ok, resolved_supervisor} <- validate_task_supervisor(task_supervisor) do
            {:ok, wrap_with_timeout(callback, timeout, resolved_supervisor)}
          end
      end
    end
  end
  ...
  defp sanitize_custom_prompt(prompt, allow_patterns?) do
    if allow_patterns? do
      ...
      if contains_injection_pattern?(prompt),
        do: {:error, :custom_prompt_injection_detected},
        else: {:ok, prompt}
    end
  end
  ...
  defp validate_task_supervisor(supervisor) when is_atom(supervisor) and not is_nil(supervisor) do
    if is_pid(Process.whereis(supervisor)),
      do: {:ok, supervisor},
      else: {:error, :missing_task_supervisor}
  end
