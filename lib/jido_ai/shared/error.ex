defmodule Jido.AI.Error do
  ...
  Invalid UTF-8 binary messages, keys and values use a `base64:` string.
  Valid UTF-8 strings are kept unchanged.
  """
  ...
  def normalize(
        %{type: type, message: message} = error,
        _fallback_type,
        _fallback_message,
        extra_details
      )
  ...
  def normalize(
        %{code: type, message: message} = error,
        _fallback_type,
        _fallback_message,
        extra_details
      )
  ...
  def normalize(%{message: message} = error, fallback_type, _fallback_message, extra_details)
    ...
    error_envelope(
      ...
      normalize_message(message),
      details,
      normalize_retryable(error, fallback_type)
    )
  end
  ...
  def normalize({:timeout, details}, _fallback_type, _fallback_message, extra_details)
    ...
    error_envelope(
      ...
      merge_error_details(details, extra_details),
      true
    )
  end

  def normalize(:timeout, _fallback_type, _fallback_message, extra_details)
      when is_map(extra_details) do
  ...
  def normalize(reason, fallback_type, fallback_message, extra_details)
      when is_map(extra_details) do
    error_envelope(
      fallback_type,
      fallback_message,
  ...
  def capture(fun) when is_function(fun, 0) do
    ...
    error -> bridge_result({:error, error})
  catch
    kind, reason -> bridge_result({:error, {kind, reason}})
  end

  defp bridge_result({:error, %Jido.Action.Error.ExecutionFailureError{details: %{jido_ai_cause: _}}} = result),
    do: result

  defp bridge_result({:error, reason}) do
    # Core Flow preserves Action errors. Carry the AI cause until the request
    # owner applies storage conversion; core execution still owns task lifetime.
    {:error,
     Jido.Action.Error.execution_error("AI operation failed", %{
       jido_ai_cause: reason,
       retry: retryable?(reason)
     })}
  end

  defp bridge_result(result), do: result

  @doc false
  def cause(%Jido.Action.Error.ExecutionFailureError{details: %{jido_ai_cause: reason}}),
    do: reason

  def cause(reason), do: reason

  @doc false
  def for_storage(error) do
    reason = cause(error)

    case Jido.Action.validate_static_data(reason) do
      :ok -> reason
      _ -> storage_error(reason)
    end
  end

  # Keep failure wrappers while converting an inner cause that has live resources.
  defp storage_error(reason) when is_tuple(reason),
    do: reason |> Tuple.to_list() |> Enum.map(&for_storage/1) |> List.to_tuple()

  defp storage_error(reason), do: normalize(reason)
  ...
  def retryable?(%{type: type} = error) when is_atom(type),
    do: retryable_hint(error, retryable_type?(type))

  def retryable?(%{code: type} = error) when is_atom(type),
    do: retryable_hint(error, retryable_type?(type))

  def retryable?(%{type: type} = error) when is_binary(type),
    do: retryable_from_string_type(error, type)

  def retryable?(%{code: type} = error) when is_binary(type),
    do: retryable_from_string_type(error, type)
  ...
  def retryable?(%module{} = error) do
    if upstream_jido_error_struct?(module), do: normalize(error).retryable?, else: false
  end

  def retryable?(reason) when is_atom(reason), do: retryable_type?(reason)
  def retryable?(_reason), do: false

  defp merge_error_details(details, extra_details)
       when is_map(details) and is_map(extra_details) do
  ...
  defp normalize_json_safe_key(key) when is_binary(key), do: json_binary(key)
  ...
  defp normalize_json_safe_value(value) when is_binary(value), do: json_binary(value)
  ...
  defp normalize_upstream_jido_error(reason, fallback_type, fallback_message, extra_details) do
    adapter =
      if String.starts_with?(Atom.to_string(reason.__struct__), "Elixir.Jido.Flow.Error."),
        do: Jido.Flow.Error,
        else: Jido.Error
  ...
  defp upstream_jido_error_struct?(module) do
    jido_error_adapter_available?() and exception_struct?(module) and
      jido_error_namespace?(module)
  end
  ...
  defp extract_nested_reason(%{} = map) do
    Map.get(map, :reason) ||
      Map.get(map, "reason") ||
      retry_hint_message(Map.get(map, :message)) ||
      retry_hint_message(Map.get(map, "message"))
  end
  ...
  defp extract_retry_value(%{} = map) do
    cond do
      ...
      Map.has_key?(map, "retry") -> Map.get(map, "retry")
      Map.has_key?(map, :retryable?) -> Map.get(map, :retryable?)
      Map.has_key?(map, "retryable?") -> Map.get(map, "retryable?")
      Map.has_key?(map, :retryable) -> Map.get(map, :retryable)
      Map.has_key?(map, "retryable") -> Map.get(map, "retryable")
      true -> nil
  ...
  defp retryable_type?(type) when type in [:timeout, :transient, :transient_error, :rate_limited],
    do: true
...
defmodule Jido.AI.Error.API.Request do
  ...
  def message(%{kind: :provider, status: status}) when is_integer(status),
    do: "Provider error (#{status})"
...
defmodule Jido.AI.Error.Validation.Output do
  ...
  def message(%{field: field}) when field in [:output, "output"],
    do: "Structured output validation failed"
