defmodule Jido.AI.ToolAdapter do
  ...
  - **Schema-focused**: Tools use a noop callback; Jido owns execution via `Jido.Exec`
  ...
  actions to the LLM. Actual execution happens via `Jido.Exec`.
  ...
  ## Options
    ...
    * `:name` - Registered tool name to use instead of the module name
  ...
  def from_action(action_module, opts) when is_atom(action_module) do
    Code.ensure_compiled!(action_module)
    prefix = Keyword.get(opts, :prefix)
    ...
    ReqLLM.Tool.new!(
      name: apply_prefix(Keyword.get(opts, :name, action_module.name()), prefix),
  ...
  defp validate_action_module(module) do
    cond do
      not match?({:module, _}, Code.ensure_compiled(module)) -> {:error, :not_loaded}
  ...
  defp valid_action_module?(module) when is_atom(module) do
    match?({:module, _}, Code.ensure_compiled(module)) and function_exported?(module, :name, 0)
  end
  ...
  @doc false
  def parameter_schema(schema, strict? \\ false), do: build_json_schema(schema, strict?)
  ...
  defp action_schema_to_json_schema(schema, strict?) do
    # ReqLLM's direct Zoi converter closes every object. Convert with Zoi first
    # so an explicitly open non-strict object keeps its contract.
    schema =
      if is_struct(schema) do
        schema
        |> Zoi.Schema.traverse(&json_type/1)
        |> json_type()
        |> Zoi.to_json_schema()
        |> Jason.encode!()
        |> Jason.decode!()
    ...
    |> close_strict_objects(strict?)
  end

  # JSON carries atom labels as strings. Keep the original Action schema for
  # validation; SchemaInput resolves only atoms that already exist on the host.
  defp json_type(%Zoi.Types.Atom{meta: meta}), do: %{Zoi.string() | meta: meta}
  defp json_type(schema), do: schema

  defp close_strict_objects(value, false), do: value

  defp close_strict_objects(value, true) when is_map(value) do
    value = Map.new(value, fn {key, item} -> {key, close_strict_objects(item, true)} end)

    if value["type"] == "object" or Map.has_key?(value, "properties"),
      do: Map.put(value, "additionalProperties", false),
      else: value
  end

  defp close_strict_objects(value, true) when is_list(value),
    do: Enum.map(value, &close_strict_objects(&1, true))

  defp close_strict_objects(value, true), do: value
