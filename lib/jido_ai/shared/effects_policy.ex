defmodule Jido.AI.Effects.Policy do
  ...
  defp normalize_constraints(%{} = constraints) do
    ...
    |> maybe_put_constraint(
      :schedule,
      normalize_schedule_constraints(fetch(constraints, :schedule, nil))
    )
  end
  ...
  defp normalize_emit_constraints(%{} = constraints) do
    ...
    |> maybe_put_constraint(
      ...
      normalize_list(fetch(constraints, :allowed_signal_prefixes, nil))
    )
    |> maybe_put_constraint(
      :allowed_signal_types,
      normalize_list(fetch(constraints, :allowed_signal_types, nil))
    )
    |> maybe_put_constraint(
      :allowed_dispatches,
      normalize_list(fetch(constraints, :allowed_dispatches, nil))
    )
  end
  ...
  defp merge_constraints(%{} = agent, %{} = strategy) do
    ...
    schedule =
      merge_schedule_constraints(fetch(agent, :schedule, nil), fetch(strategy, :schedule, nil))
  ...
  defp emit_type_allowed?(%Directive.Emit{signal: signal}, constraints) do
    ...
    prefix_allowed? =
      case allowed_prefixes do
        ...
        prefixes ->
          is_binary(signal_type) and Enum.any?(prefixes, &String.starts_with?(signal_type, &1))
      end
  ...
  defp schedule_delay_allowed?(%Schedule{delay_ms: delay_ms}, constraints) do
    case fetch(constraints, :max_delay_ms, nil) do
