defmodule Jido.AI.PendingInputServer do
  ...
  The server monitors its declared owner. A native Session creates and owns
  its queue. Standalone ReAct can borrow a caller-owned queue through Config.
  Inputs are queued synchronously and drained before model turns. A terminal
  run seals the queue to reject late input. It stops only queues it created.
  """
