defmodule Jido.AI.Query do
  ...
  def summarize(parts) when is_list(parts) do
    Enum.map_join(parts, "\n", fn
      ...
      when type in @file_part_type_values and is_binary(filename) ->
        "[File: #{filename}]"
      ...
      %{"type" => type, "filename" => filename}
      when type in @file_part_type_strings and is_binary(filename) ->
        "[File: #{filename}]"
  ...
  defp normalize_file_reference(_reference),
    do: {:error, {:invalid_file_reference, :missing_file_id}}
