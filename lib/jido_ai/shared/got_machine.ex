defmodule Jido.AI.Reasoning.GraphOfThoughts.Machine do
  ...
  Returned work descriptions do not execute model requests.
  ...
  **External (Strategy state, Snapshots):** Status is converted to atoms (`:idle`,
  ...
  def update(%__MODULE__{status: "idle"} = machine, {:start, prompt, call_id}, _env) do
    emit_telemetry(machine, :start, %{prompt_length: String.length(prompt)})
  ...
  defp trace_path_recursive(machine, node_id, root_id, path) do
    if node_id in path do
      ...
      case get_parents(machine, node_id) do
        [] ->
          [node_id | path]

        parents ->
          Enum.find_value(parents, [], fn parent_id ->
            case trace_path_recursive(machine, parent_id, root_id, [node_id | path]) do
              [] -> nil
              path -> path
            end
          end)
  ...
  defp handle_error(machine, reason) do
    case transition(machine, "error") do
      {:ok, machine} ->
        machine =
          machine
        ...
        emit_telemetry(machine, :error, %{reason: reason})
        {machine, []}
  ...
  defp continue_generation(machine, node_id, _env) do
    ...
    machine =
      if machine.status == "generating" do
        ...
        case transition(machine, "generating") do
          {:ok, m} -> m
          {:error, _} -> machine
        end
      end
  ...
  defp complete_with_result(machine, result) do
    case transition(machine, "completed") do
      {:ok, machine} ->
        ...
        emit_telemetry(machine, :complete, %{
  ...
  defp accumulate_usage(machine, result) do
    incoming =
      (Jido.AI.Usage.normalize(Map.get(result, :usage)) || %{})
      |> Map.put_new(:input_tokens, 0)
      |> Map.put_new(:output_tokens, 0)
      |> Jido.AI.Usage.ensure_total_tokens()

    %{machine | usage: Jido.AI.Usage.merge(machine.usage, incoming)}
  end

  defp emit_telemetry(%{emit_telemetry?: false}, _, _), do: :ok

  defp emit_telemetry(_machine, event, metadata) do
    :telemetry.execute(
