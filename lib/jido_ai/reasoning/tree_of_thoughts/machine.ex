defmodule Jido.AI.Reasoning.TreeOfThoughts.Machine do
  ...
  The returned tuples describe work. They do not execute model or tool calls.

  ## Status Type Boundary

  **Internal (Machine struct):** Status is stored as strings (`"idle"`, `"completed"`)
  to retain the existing data format.
  ...
  **External (Strategy state, Snapshots):** Status is converted to atoms (`:idle`,
  ...
  def update(%__MODULE__{status: "idle"} = machine, {:start, prompt, call_id}, env) do
    ...
    emit_telemetry(machine, :start, %{system_time: System.system_time()}, %{
      call_id: call_id,
  ...
  def update(
        %__MODULE__{status: "generating"} = machine,
        {:thoughts_generated, call_id, thoughts},
        _env
      ) do
  ...
  def update(
        %__MODULE__{status: "evaluating"} = machine,
        {:thoughts_evaluated, call_id, scores},
        env
      ) do
    if call_id == machine.current_call_id do
  ...
  defp transition(machine, status) do
    if status in Map.get(@transitions, machine.status, []),
      do: {:ok, %{machine | status: status}},
      else: {:error, :invalid_transition}
  end

  defp with_transition(machine, new_status, fun) do
    case transition(machine, new_status) do
      {:ok, machine} -> fun.(machine)
      {:error, _} -> {machine, []}
    end
  end

  defp bounded_entries(machine, thoughts) do
    count = min(machine.branching_factor, max(machine.max_nodes - map_size(machine.nodes), 0))
    thoughts |> normalize_thought_entries() |> Enum.take(count)
  end

  defp handle_thoughts_generated(machine, thoughts) when is_list(thoughts) do
    thought_entries = bounded_entries(machine, thoughts)
  ...
  defp handle_thoughts_evaluated(machine, scores, env) when is_map(scores) do
    thought_entries = bounded_entries(machine, machine.pending_thoughts)
    ...
    if thought_entries == [] do
      ...
      machine =
        machine
        |> update_in(
          [Access.key(:nodes), Access.key(machine.current_node_id), Access.key(:children)],
          fn _ ->
            child_ids
          end
        )
  ...
  defp start_generating_for_node(machine, node_id, remaining_frontier, env) do
    ...
    with {:ok, machine} <- transition(machine, "expanding"),
         machine = Map.put(machine, :frontier, remaining_frontier),
         {:ok, machine} <- transition(machine, "generating") do
      call_id = generate_call_id()
  ...
  defp max_duration_exceeded?(%__MODULE__{max_duration_ms: ms} = machine)
  ...
  defp error_result(%__MODULE__{} = machine, reason) do
    ...
    |> put_in([:termination, :status], "error")
    |> put_in([:diagnostics, :error], inspect(reason))
    |> put_in([:diagnostics, :cause], reason)
  end

  @doc false
  def diagnostics(%__MODULE__{} = machine, best_node_id) do
    %{
  ...
  defp accumulate_usage(machine, result) do
    case Map.get(result, :usage) do
      ...
      new_usage when is_map(new_usage) ->
        %{machine | usage: Jido.AI.Usage.merge(machine.usage, new_usage)}
    end
  end

  defp emit_telemetry(%{emit_telemetry?: false}, _, _, _), do: :ok

  defp emit_telemetry(_machine, event, measurements, metadata) do
