defmodule Jido.AI.Reasoning.ReAct do
  ...
  """
  @spec stream(String.t() | [ReqLLM.Message.ContentPart.t()], config_input(), keyword()) ::
          Enumerable.t()
  def stream(query, config, opts \\ [])
      when (is_binary(query) or is_list(query)) and is_list(opts) do
  ...
  def continue(checkpoint_token, config, opts \\ [])
      when is_binary(checkpoint_token) and is_list(opts) do
    config = build_config(config)
  ...
  @spec collect(Enumerable.t() | String.t(), config_input(), keyword()) ::
          {:ok, map()} | {:error, term()}
  def collect(checkpoint_token, config, opts)
      when is_binary(checkpoint_token) and is_list(opts) do
  ...
  defp update_collect_from_event(acc, %{kind: :checkpoint, data: %{token: token}}),
    do: %{acc | final_token: token}
  ...
  defp update_collect_from_event(acc, %{kind: :request_failed, data: data}) do
    %{
      acc
      | result: Map.get(data, :error),
        termination_reason: :failed,
        usage: terminal_collect_usage(acc.usage, Map.get(data, :usage))
    }
  end

  defp update_collect_from_event(acc, %{kind: :request_cancelled, data: data}) do
    %{
      acc
