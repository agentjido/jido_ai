defmodule Jido.AI.Reasoning.ReAct.Token do
  @moduledoc """
  ...
  def issue(%State{} = state, %Config{} = config) do
    ...
    case validate_saved_state(payload) do
      :ok -> :ok
      {:error, reason} -> raise ArgumentError, "invalid ReAct checkpoint: #{inspect(reason)}"
    end
  ...
  defp validate_saved_state(payload) do
    with :ok <- portable_state(payload.state),
         {:ok, state} <- State.from_checkpoint_map(payload.state),
         true <- state.request_id == payload[:request_id] and state.run_id == payload[:run_id],
         :ok <- Jido.AI.Reasoning.ReAct.Checkpoint.validate_state(state) do
      :ok
    else
      false -> {:error, :token_identity_mismatch}
      {:error, _} = error -> error
    end
  end

  defp portable_state(state) do
    case Jido.Action.validate_static_data(state) do
      :ok -> :ok
      {:error, _} -> {:error, :nonportable_token_state}
    end
  end
