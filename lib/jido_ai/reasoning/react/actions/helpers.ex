defmodule Jido.AI.Reasoning.ReAct.Actions.Helpers do
  ...
  def build_config(params, context) do
    ...
    opts = %{
      ...
      llm_timeout_ms: params[:llm_timeout_ms],
  ...
  def build_runner_opts(params, context) do
    opts =
      ...
      |> maybe_put(:limits, params[:limits])
  ...
  defp context_task_supervisor(context) when is_map(context) do
    Map.get(context, :task_supervisor) ||
      ...
      context_task_supervisor(Map.get(context, :state)) ||
      context_task_supervisor(Map.get(context, :agent))
  end
  ...
  defp resolve_tools!(params, context) do
    ...
    case ToolSelection.resolve(normalize_base_tools!(base_tools), override_tools, allowed_tools) do
      ...
      {:error, :invalid_tools} ->
        raise ArgumentError,
              "invalid ReAct tools: expected a tool map, module, or list of action modules"
