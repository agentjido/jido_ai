defmodule Jido.AI.Reasoning.TRM.Machine do
  @moduledoc """
  ...
  **External (Strategy state, Snapshots):** Status is converted to atoms (`:idle`,
  ...
  defp handle_reasoning_result(machine, {:ok, result}) do
    ...
    emit_telemetry(machine, :step, %{system_time: System.system_time()}, %{
    ...
    if should_terminate_max_steps?(machine) do
      ...
      case ACT.make_decision(machine.act_state, machine.latent_state) do
        {:halt, reason} ->
          ...
          emit_telemetry(machine, :act_triggered, %{system_time: System.system_time()}, %{
        ...
        {:continue, %{expected_improvement: expected}} ->
          emit_telemetry(machine, :act_continue, %{system_time: System.system_time()}, %{
            confidence: machine.act_state.current_confidence,
            threshold: machine.act_threshold,
            step: machine.supervision_step,
  ...
  defp handle_error(machine, reason) do
    ...
    emit_telemetry(machine, :error, %{duration: duration_ms}, %{
      step: machine.supervision_step,
      error: reason,
      usage: machine.usage
    })
  ...
  def from_map(map) when is_map(map) do
    ...
    %__MODULE__{
      ...
      emit_telemetry?: Map.get(map, :emit_telemetry?, true)
    }
  end
  ...
  defp with_transition(machine, new_status, fun) do
    if new_status in Map.fetch!(@transitions, machine.status),
      do: fun.(%{machine | status: new_status}),
      else: {machine, []}
  end

  defp accumulate_usage(machine, result) do
    case Jido.AI.Usage.normalize(Map.get(result, :usage)) do
      nil ->
        machine

      incoming ->
        incoming = Jido.AI.Usage.ensure_total_tokens(incoming)
