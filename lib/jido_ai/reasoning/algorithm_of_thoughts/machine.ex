defmodule Jido.AI.Reasoning.AlgorithmOfThoughts.Machine do
  ...
  def update(
        %__MODULE__{status: "exploring"} = machine,
  ...
  defp with_transition(machine, new_status, fun) do
    case {machine.status, new_status} do
      ...
      {"exploring", status} when status in ["completed", "error"] ->
        fun.(%{machine | status: new_status})
  ...
  defp normalize_usage(usage) when is_map(usage), do: Jido.AI.Usage.with_token_counts(usage)
  ...
  defp duration_ms(%__MODULE__{started_at: started_at}),
  ...
  @doc "Builds the AoT user prompt used by the shared model operation."
  def user_prompt(prompt) when is_binary(prompt) do
    """
