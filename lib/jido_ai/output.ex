defmodule Jido.AI.Output do
  ...
  def validate(%__MODULE__{schema_kind: :zoi, schema: schema}, value) when is_map(value) do
    value = Jido.AI.SchemaInput.normalize(schema, value)
  ...
  def validate(%__MODULE__{schema_kind: :json_schema, schema: schema}, value)
  ...
  def json_schema(%__MODULE__{schema_kind: :zoi, schema: schema}),
    do: ReqLLM.Schema.to_json(schema)
  ...
  @doc false
  def mark_failed(meta, reason) do
    meta
    |> Map.delete(:validation_error)
    |> Map.merge(%{status: :error, error: format_meta_error(reason)})
  end
  ...
  defp default_repair(output, raw, reason, context) do
    ...
    if is_nil(request.model) do
      ...
      case ReqLLM.Generation.generate_object(
             request.model,
             request.messages,
             output.schema,
             request.llm_opts
  ...
  defp invoke_repair({module, function}, output, raw, reason, context) do
    cond do
      function_exported?(module, function, 4) ->
        apply(module, function, [output, raw, reason, context])

      function_exported?(module, function, 3) ->
        apply(module, function, [output, raw, reason])
  ...
  defp append_system_prompt(messages, addition),
    do: [%{role: :system, content: addition} | messages]
  ...
  defp normalize_retries(value) when is_integer(value) and value >= 0,
    do: {:ok, min(value, @max_retries)}
  ...
  defp normalize_mode(_value),
    do: {:error, "output on_validation_error must be :repair or :error"}
  ...
  defp format_meta_error(%{details: details}) do
    %{type: :output_error, message: "Output failed", details: details}
    |> Error.normalize()
    |> Map.fetch!(:details)
    |> Jido.AI.Observe.Sanitize.transport_payload()
  end

  defp format_meta_error(reason),
    do: inspect(reason, limit: 20, printable_limit: @raw_preview_bytes)
  ...
  defp sanitize_preview_value(value), do: Jido.AI.Observe.Sanitize.sensitive(value)
end
