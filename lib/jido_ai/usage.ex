defmodule Jido.AI.Usage do
  ...
  def token_counts(usage) when is_map(usage) do
    ...
    total_tokens =
      first_token_value(usage_sources, @total_token_keys) || input_tokens + output_tokens
  ...
  defp merge_metadata_value(left, right) when is_map(left) and is_map(right),
    do: merge(left, right)
  ...
  defp numeric_usage_key?(key) when is_atom(key),
    do: key |> Atom.to_string() |> numeric_usage_key?()
