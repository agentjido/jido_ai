...
if [ ! -z "$JIDO_WORKTREE_OVERRIDE_BASE" ]; then
    WORKTREE_DIR_NAME="${WORKTREE_NAME}"
    WORKTREES_BASE="${JIDO_WORKTREE_OVERRIDE_BASE}/${REPO_BASE_NAME}"
else
    WORKTREE_DIR_NAME="${WORKTREE_NAME}"
    WORKTREES_BASE="$HOME/worktrees/${REPO_BASE_NAME}"
fi
...
cd "$WORKTREE_PATH"
if humanlayer thoughts init --directory jido_workspace > /dev/null 2>&1; then
