...
| `AUT-REQ-028` | Implemented; evidence incomplete | Related example evidence (partial): [DSL, source profiles, source JSON and Builder use the same session targets](../../../test/examples/02_requests/02_01_session/02_01_session_test.exs). | Verify the target behavior: An authored AI Agent shall expose Profile inspection and the request functions supported by its request mode. |
