...
| --- | --- | --- | --- |
| `SES-REQ-001` | Implemented; evidence incomplete | Related example evidence (partial): [restored request records reject runtime resources and mismatched IDs](../../../test/examples/02_requests/02_01_session/02_01_session_test.exs). | Verify the target behavior: Every AI request shall have a nonempty request ID and run ID before admission commits. |
| `SES-REQ-002` | Decision required | Reconcile run-ID wording with the selected request/attempt meanings. A ReAct run ID is not a unique execution attempt. Keep logical identity and select public attempt fields with seam 11. Related example evidence (partial): [restored request records reject runtime resources and mismatched IDs](../../../test/examples/02_requests/02_01_session/02_01_session_test.exs). | Verify the target behavior: A request ID shall identify the logical request and a run ID shall identify one live execution attempt. |
| `SES-REQ-003` | Implemented; evidence incomplete | Related example evidence (partial): [busy and duplicate IDs reject before work and preserve the first stream](../../../test/examples/02_requests/02_01_session/02_01_session_test.exs). | Verify the target behavior: When the same request ID already exists in retained Agent state, admission shall reject the request as a duplicate. |
...
| `SES-REQ-014` | Implemented; evidence incomplete | Related example evidence (partial): [admission commits before work and the final Turn preserves intervening domain changes](../../../test/examples/02_requests/02_01_session/02_01_session_test.exs); [a result that violates the domain schema becomes a committed failure](../../../test/examples/02_requests/02_01_session/02_01_session_test.exs). | Verify the target behavior: A successful settlement shall assemble the complete candidate Agent, update the declared result and history fields, update the request record, and return approved Directives in one Turn result. |
| `SES-REQ-015` | Implemented; evidence incomplete | Related example evidence (partial): [admission commits before work and the final Turn preserves intervening domain changes](../../../test/examples/02_requests/02_01_session/02_01_session_test.exs); [a result that violates the domain schema becomes a committed failure](../../../test/examples/02_requests/02_01_session/02_01_session_test.exs). | Verify the target behavior: A failed settlement shall preserve the prior domain state except for the terminal request record and approved failure metadata. |
| `SES-REQ-016` | Implemented; evidence incomplete | Related example evidence (partial): [a post-commit directive failure preserves the answer and is not retried](../../../test/examples/02_requests/02_11_completion/02_11_completion_test.exs). | Verify the target behavior: A committed request outcome shall be authoritative even if a later terminal stream item cannot be delivered. |
| `SES-REQ-017` | Implemented; evidence incomplete | Related example evidence (partial): [completed requests have bounded retention and await_many keeps input order](../../../test/examples/02_requests/02_01_session/02_01_session_test.exs). | Verify the target behavior: Request retention shall never evict pending work and shall use a deterministic order for terminal record eviction. |
| `SES-REQ-018` | Implemented; evidence incomplete | Related example evidence (partial): [caller and consumer timeouts leave the accepted work alive](../../../test/examples/02_requests/02_01_session/02_01_session_test.exs). | Verify the target behavior: `await/2` shall use public AgentServer or session APIs and shall select the record by request ID. |
| `SES-REQ-019` | Implemented; evidence incomplete | Related example evidence (partial): [caller and consumer timeouts leave the accepted work alive](../../../test/examples/02_requests/02_01_session/02_01_session_test.exs). | Verify the target behavior: An await timeout shall not change the committed request status or cancel the request unless the caller explicitly requests cancellation. |
| `SES-REQ-020` | Implemented; evidence incomplete | Related example evidence (partial): [real SSE text and request headers precede the final commit](../../../test/examples/02_requests/02_01_session/02_01_session_test.exs). | Verify the target behavior: A request stream shall be an ordered best-effort live view and shall not be described as a durable event log. |
| `SES-REQ-021` | Implemented; evidence incomplete | Related example evidence (partial): [real SSE text and request headers precede the final commit](../../../test/examples/02_requests/02_01_session/02_01_session_test.exs). | Verify the target behavior: A request stream shall contain exactly one terminal item when its sink remains available through termination. |
| `SES-REQ-022` | Implemented; evidence incomplete | Target scenario passed: [SES-REQ-022 target check](../../../test/examples/02_requests/02_01_session/design_requirements_test.exs). The accepted request completes after its stream sink terminates. | Verify the target behavior: If a stream sink terminates, the request shall continue unless profile policy explicitly requires stream ownership. |
| `SES-REQ-023` | Implemented; evidence incomplete | Related example evidence (partial): [cancellation stops a held real tool and cannot stop the next request](../../../test/examples/02_requests/02_01_session/02_01_session_test.exs). | Verify the target behavior: A cancellation request shall name a request ID or resolve unambiguously to the single pending request. |
| `SES-REQ-024` | Decision required | Related example evidence (partial): [cancellation stops a held real tool and cannot stop the next request](../../../test/examples/02_requests/02_01_session/02_01_session_test.exs). | Verify the target behavior: If cancellation commits before settlement, the record shall become `:cancelled`, pending continuations shall stop, and later settlement shall be stale. |
| `SES-REQ-025` | Decision required | Related example evidence (partial): [cancellation stops a held real tool and cannot stop the next request](../../../test/examples/02_requests/02_01_session/02_01_session_test.exs). | Verify the target behavior: If settlement commits before cancellation, cancellation shall return `:request_already_finished` and shall not replace the terminal result. |
| `SES-REQ-026` | Implemented; evidence incomplete | Related example evidence (partial): [a failed Agent result commit does not undo the external memory write](../../../test/examples/07_retrieval/07_01_memory/07_01_memory_test.exs). | Verify the target behavior: The cancellation contract shall state that it cannot roll back a model call, tool call, or other external effect that completed before cancellation. |
| `SES-REQ-027` | Implemented; evidence incomplete | Related example evidence (partial): [steer and inject during a real tool keep one handle, FIFO input and committed consumption](../../../test/examples/02_requests/02_02_steering/02_02_steering_test.exs). | Verify the target behavior: Steering and injection shall be available only for a method and profile that declare active-input support. |
| `SES-REQ-028` | Implemented; evidence incomplete | Related example evidence (partial): [steer and inject during a real tool keep one handle, FIFO input and committed consumption](../../../test/examples/02_requests/02_02_steering/02_02_steering_test.exs). | Verify the target behavior: A control item shall include a control ID, kind, visible content, expected request ID, source, and bounded references. |
| `SES-REQ-029` | Implemented; evidence incomplete | Target scenario passed: [SES-REQ-029 target check](../../../test/examples/02_requests/02_02_steering/design_requirements_test.exs). Queued steering is absent before consumption and appears after it is consumed. Related example evidence (partial): [steer and inject during a real tool keep one handle, FIFO input and committed consumption](../../../test/examples/02_requests/02_02_steering/02_02_steering_test.exs). | Verify the target behavior: A control acknowledgement shall state whether the item was queued, rejected, or applied; a queued result shall not claim consumption. |
| `SES-REQ-030` | Implemented; evidence incomplete | Related example evidence (partial): [the default queue admits 64 inputs and keeps FIFO order](../../../test/examples/02_requests/02_02_steering/02_02_steering_test.exs). | Verify the target behavior: The live control queue shall be bounded and shall not be stored as a generic Agent mailbox. |
| `SES-REQ-031` | Implemented; evidence incomplete | Related example evidence (partial): [after-tools checkpoint preserves completed tool history and does not repeat its side effect](../../../test/examples/14_resume/14_03_checkpoint_resume/14_03_checkpoint_resume_test.exs). | Verify the target behavior: Standalone request APIs shall use the same public Agent, Plugin, Flow, Exec, and request contracts as authored Agents. |
| `SES-REQ-032` | Implemented; evidence incomplete | Related example evidence (partial): [after-tools checkpoint preserves completed tool history and does not repeat its side effect](../../../test/examples/14_resume/14_03_checkpoint_resume/14_03_checkpoint_resume_test.exs). | Verify the target behavior: A standalone facade shall own and stop any local AgentServer that it creates. |
| `SES-REQ-033` | Implemented; evidence incomplete | Related example evidence (partial): [a held model has live identity and a separate committed request](../../../test/examples/02_requests/02_22_request_inspection/02_22_request_inspection_test.exs). | Verify the target behavior: Session inspection shall separate the committed request record from transient live samples. |
| `SES-REQ-034` | Implemented; evidence incomplete | Related example evidence (partial): [the trace cap keeps the first 2000 events and records overflow without hiding completion](../../../test/examples/02_requests/02_22_request_inspection/02_22_request_inspection_test.exs). | Verify the target behavior: Inspection shall bound trace and metadata size and shall mark truncated data. |
| `SES-REQ-035` | Proposed; not implemented | To be implemented: linked delegation example for parent and delegated request correlation with target Agent identity. Current inert subagent/handoff declarations are not an executable delegation API. | Verify the target behavior: When an Agent delegates work, Orchestration shall correlate the delegated request with its originating request and target Agent identity. |
| `SES-REQ-036` | Proposed; not implemented | To be implemented: linked delegation example for policy-limited context transfer. Current inert subagent/handoff declarations are not an executable delegation API. | Verify the target behavior: When Orchestration sends delegated work, it shall transfer only context selected by the configured delegation policy. |
| `SES-REQ-037` | Proposed; not implemented | To be implemented: linked delegation example for parent acceptance policy before Thread commit. Current inert subagent/handoff declarations are not an executable delegation API. | Verify the target behavior: When a delegated result arrives, the originating Orchestration shall apply the parent request policy before proposing a parent Thread update. |
| `SES-REQ-038` | Proposed; not implemented | To be implemented: linked delegation example for late-result rejection after parent settlement. Current inert subagent/handoff declarations are not an executable delegation API. | Verify the target behavior: If the parent request is terminal when a delegated result arrives, then Orchestration shall apply an explicit late-result policy without silently replacing the committed outcome. |
| `SES-REQ-039` | Proposed; not implemented | To be implemented: linked delegation example for peer-work cancellation without stopping the peer Agent. Current inert subagent/handoff declarations are not an executable delegation API. | Verify the target behavior: When a caller cancels delegated work on a peer, Orchestration shall cancel only the selected work unless the host separately authorizes stopping that Agent. |
| `SES-REQ-040` | Proposed; not implemented | To be implemented: linked delegation example for depth, concurrency, and shared-budget admission limits. Current inert subagent/handoff declarations are not an executable delegation API. | Verify the target behavior: Where delegation is enabled, Orchestration shall validate finite depth, concurrency, and request-budget limits before admitting delegated work. |
