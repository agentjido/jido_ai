...
| `CAP-REQ-007` | Implemented; evidence incomplete | Related example evidence (partial): [two tool rounds preserve options usage and one assistant message per response](../../../test/examples/16_capabilities/16_02_chat/16_02_chat_test.exs). | Verify the target behavior: The Chat capability shall store context history only in the Agent domain field declared by the author. |
