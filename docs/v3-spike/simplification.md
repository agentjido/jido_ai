...
| Remove; complete | Install, skill, and quality Mix tasks and Igniter | The package no longer ships Mix tasks. Runtime skill APIs remain; repository-local Mix aliases still run developer checks. |
...
| Move; proposed | Historical API and CLI obligations | Old audit files still describe the CLI as retained work. Keep historical evidence separate from the current public API map and dated source snapshot. This record supersedes their CLI retention decision. |
