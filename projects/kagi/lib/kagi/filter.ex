defmodule Kagi.Filter do
  ...
  defp sensitive_patterns do
    [
      ...
      ~r/(^|[^a-z0-9])auth(orization)?($|[^a-z0-9])/i,
      ...
      ~r/(^|[^a-z0-9])cert(ificate)?($|[^a-z0-9])/i,
  ...
  def format(level, message, _ts, metadata) do
    ...
    # Convert message to string if it's not already iodata
    msg_str =
      case msg do
        m when is_binary(m) or is_list(m) -> m
        m -> inspect(m)
      end

    ["[", to_string(level), "] ", msg_str, " ", inspect(md), "\n"] |> IO.iodata_to_binary()
  end
  ...
  def sanitize_data(data) when is_list(data) do
    if Keyword.keyword?(data) do
      ...
      sanitize_regular_list(data)
    end
  end

  def sanitize_data(data) when is_tuple(data) do
    case data do
      # Handle key-value tuples specially
      ...
      # Handle other tuples generically
      _ ->
        data
        |> Tuple.to_list()
  ...
  @spec sanitize_keyword_list(keyword()) :: keyword()
  defp sanitize_keyword_list(data) do
  ...
  @spec sanitize_regular_list(list()) :: list()
  defp sanitize_regular_list(data) do
  ...
  def looks_like_sensitive_value?(value) when is_binary(value) do
    long_base64_like?(value) or
      jwt_like?(value) or
      has_known_token_prefix?(value) or
      aws_access_key?(value) or
      openai_key?(value)
  end

  def looks_like_sensitive_value?(_), do: false

  @spec long_base64_like?(binary()) :: boolean()
  defp long_base64_like?(value) do
    String.length(value) > 20 and Regex.match?(~r/^[A-Za-z0-9+\/=_-]+$/, value)
  end

  @spec jwt_like?(binary()) :: boolean()
  defp jwt_like?(value) do
    String.length(value) > 50 and String.contains?(value, ".") and
      Regex.match?(~r/^[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+$/, value)
  end

  @spec has_known_token_prefix?(binary()) :: boolean()
  defp has_known_token_prefix?(value) do
    Regex.match?(~r/^(gh[pousr]_|glpat-|gho_|ghu_|ghs_|ghr_)/, value)
  end

  @spec aws_access_key?(binary()) :: boolean()
  defp aws_access_key?(value) do
    Regex.match?(~r/^AKIA[0-9A-Z]{16}$/, value)
  end

  @spec openai_key?(binary()) :: boolean()
  defp openai_key?(value) do
    Regex.match?(~r/^sk-[A-Za-z0-9]{32,}$/, value)
  end

  @spec sanitize_logger_message(Logger.message()) :: term()
  ...
  defp sanitize_logger_message(other) do
    other |> to_string() |> sanitize_data()
  rescue
    Protocol.UndefinedError ->
      # For complex structures that don't implement String.Chars
      sanitize_data(other)
  end
end
