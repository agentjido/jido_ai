defmodule Kagi do
  @moduledoc """
  Centralized configuration and secret management system.

  Kagi provides a unified API for managing environment variables, API keys,
  and configuration settings with hierarchical loading and process-specific overrides.
  ...
  3. Application environment (under :kagi, :keyring)
  4. Default values
  ...
  ## Usage
      ...
      api_key = Kagi.get(:openai_api_key, "default")

      # Set a session override (process-specific)
      ...
      value = Kagi.get(:my_key, "default")

      # Set a session-specific override
      Kagi.set_session_value(:my_key, "override")

      # Clear session values
      Kagi.clear_session_value(:my_key)
  ...
  @session_registry :kagi_keyring_sessions
  @default_name __MODULE__
  @default_env_table_prefix :kagi_env_cache
  ...
  defp get_environment do
    # Check application config first, falling back to default
    Application.get_env(:kagi, :env) || detect_mix_env() || :prod
  end

  @spec detect_mix_env() :: atom() | nil
  ...
  @spec do_env_lookup(atom(), atom() | String.t(), term()) :: term()
  ...
  """
  @spec set_test_env_vars(map(), GenServer.server()) :: :ok
