defmodule Kagi.ServerTest do
  @moduledoc """
  Tests for the Kagi.Server GenServer.
  """
  ...
  describe "GenServer lifecycle" do
    test "starts and stops correctly" do
      assert {:ok, pid} = Kagi.Server.start_link([])
  ...
  describe "environment loading" do
    test "loads environment variables" do
      with_env_vars(%{"TEST_SERVER_KEY" => "server_value"}, fn ->
        start_kagi_server()
        result = GenServer.call(Kagi.Server, {:get, :test_server_key, nil})
    ...
    test "resolves values in correct priority order" do
      ...
      with_app_config(config, fn ->
        with_env_vars(%{"TEST_PRIORITY_KEY" => "env_value"}, fn ->
          start_kagi_server()
          result = GenServer.call(Kagi.Server, {:get, :test_priority_key, nil})
  ...
  describe "key normalization" do
    test "normalizes keys consistently" do
      with_env_vars(%{"Test-Key_123" => "normalized_value"}, fn ->
        ...
        result1 = GenServer.call(Kagi.Server, {:get, :test_key_123, nil})
        result2 = GenServer.call(Kagi.Server, {:get, "test_key_123", nil})
  ...
  describe "GenServer call handling" do
    test "handles get calls correctly" do
      with_env_vars(%{"EXISTING_KEY" => "existing_value"}, fn ->
        ...
        result = GenServer.call(Kagi.Server, {:get, :existing_key, nil})
        assert result == "existing_value"

        result = GenServer.call(Kagi.Server, {:get, :missing_key, "default"})
    ...
    test "handles list calls" do
      with_env_vars(%{"KEY1" => "value1", "KEY2" => "value2"}, fn ->
        ...
        keys = GenServer.call(Kagi.Server, :list)
        assert is_list(keys)
        assert "key1" in keys
        assert "key2" in keys
      end)
  ...
  describe "reload functionality" do
    test "reloads configuration" do
      ...
      with_env_vars(%{"RELOAD_TEST_KEY" => "new_value"}, fn ->
        GenServer.cast(Kagi.Server, {:reload, []})
        :timer.sleep(50)

        result = GenServer.call(Kagi.Server, {:get, :reload_test_key, nil})
