defmodule Jido.AI.Plugins.Reasoning.ChainOfThoughtTest do
  ...
  alias Jido.AI.Plugins.Reasoning.ChainOfThought, as: Capability

  defmodule Overwrite do
    use Jido.Action, name: "overwrite_cot_defaults"

    def run(_, context) do
      next = put_in(context.agent_state.reasoning_cot, %{forged: true})
      {:ok, next}
    end
  end
  ...
  test "construction validates the Profile and fixed method" do
    profile = profile()
    {:reasoning_cot, schema} = Capability.Agent.state_spec(profile: profile)
    ...
    other = put_in(profile.reasoning, %{method: :chain_of_draft, model: :default})
