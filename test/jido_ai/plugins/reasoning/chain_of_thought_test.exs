defmodule Jido.AI.Plugins.Reasoning.ChainOfThoughtTest do
  ...
  defmodule Overwrite do
    ...
    def run(_, context) do
      next = put_in(context.agent_state.reasoning_cot.timeout, 1)
      {:ok, next}
    end
  end
  ...
  test "configured defaults survive an empty restored state and reject another method" do
    ...
    {:reasoning_cot, schema} = Capability.Agent.state_spec(config)
  ...
  test "an ordinary Action cannot replace the owned defaults" do
    ...
    assert agent.state.reasoning_cot.timeout == 30_000
  end
