defmodule Jido.AI.Plugins.Reasoning.AlgorithmOfThoughtsTest do
  ...
  alias Jido.AI.Plugins.Reasoning.AlgorithmOfThoughts, as: Capability

  defmodule Overwrite do
    use Jido.Action, name: "overwrite_aot_defaults"

    def run(_, context) do
      next = put_in(context.agent_state.reasoning_aot.timeout, 1)
      {:ok, next}
    end
  end
  ...
  test "core creates owned defaults for the fixed method" do
    ...
    assert agent.state.reasoning_aot == %{
             strategy: :aot,
  ...
  test "configured defaults survive an empty restored state and reject another method" do
    ...
    {:reasoning_aot, schema} = Capability.Agent.state_spec(config)
  ...
  test "an ordinary Action cannot replace the owned defaults" do
    ...
    assert agent.state.reasoning_aot.timeout == 30_000
  end

  defp definition(config) do
    Jido.Agent.new!(%{
      name: "aot_capability_contract",
