defmodule Jido.AI.Plugins.Reasoning.TRMTest do
  ...
  alias Jido.AI.Plugins.Reasoning.TRM, as: Capability

  defmodule Overwrite do
    use Jido.Action, name: "overwrite_trm_defaults"

    def run(_, context) do
      next = put_in(context.agent_state.reasoning_trm.timeout, 1)
      {:ok, next}
    end
  end
  ...
  test "core creates owned defaults for the fixed method" do
    ...
    assert agent.state.reasoning_trm == %{
             strategy: :trm,
  ...
  test "configured defaults survive an empty restored state and reject another method" do
    ...
    {:reasoning_trm, schema} = Capability.Agent.state_spec(config)
  ...
  test "an ordinary Action cannot replace the owned defaults" do
    ...
    assert agent.state.reasoning_trm.timeout == 30_000
  end

  defp definition(config) do
    Jido.Agent.new!(%{
      name: "trm_capability_contract",
