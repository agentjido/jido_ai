defmodule Jido.AI.Directive.HelperTest do
  ...
  describe "classify_error/1" do
    test "classifies rate limit by status" do
      error = %{status: 429}
      assert Helper.classify_error(error) == :rate_limit
    end

    test "classifies auth by status 401" do
      error = %{status: 401}
      assert Helper.classify_error(error) == :auth
    end
    ...
    test "classifies provider error by 5xx status" do
      assert Helper.classify_error(%{status: 500}) == :provider_error
      assert Helper.classify_error(%{status: 503}) == :provider_error
    end

    test "classifies validation by 4xx status" do
      assert Helper.classify_error(%{status: 400}) == :validation
      assert Helper.classify_error(%{status: 422}) == :validation
    end

    test "classifies timeout by reason" do
      assert Helper.classify_error(%{reason: :timeout}) == :timeout
      assert Helper.classify_error(%{reason: :connect_timeout}) == :timeout
      assert Helper.classify_error(%{reason: :checkout_timeout}) == :timeout
    end

    test "classifies network errors by reason" do
      assert Helper.classify_error(%{reason: :econnrefused}) == :network
      assert Helper.classify_error(%{reason: :nxdomain}) == :network
      assert Helper.classify_error(%{reason: :closed}) == :network
    end

    test "classifies error tuples" do
      assert Helper.classify_error({:error, :timeout}) == :timeout
      assert Helper.classify_error(:timeout) == :timeout
    end

    test "classifies Mint errors as network" do
      assert Helper.classify_error(%Mint.TransportError{reason: :closed}) == :network
      assert Helper.classify_error(%Mint.HTTPError{reason: :protocol_error}) == :network
    end

    test "classifies unknown errors" do
      assert Helper.classify_error(%{unknown: :error}) == :unknown
      assert Helper.classify_error("some string") == :unknown
    end
  end

  describe "resolve_directive_model/1" do
    test "returns model string directly" do
      assert Helper.resolve_directive_model(%{model: "anthropic:claude-haiku-4-5"}) ==
               "anthropic:claude-haiku-4-5"
    end

    test "resolves model alias" do
      assert Helper.resolve_directive_model(%{model_alias: :fast}) ==
               "anthropic:claude-haiku-4-5"
    end

    test "raises when neither model nor model_alias provided" do
      assert_raise ArgumentError, fn ->
        Helper.resolve_directive_model(%{model: nil, model_alias: nil})
      end

      assert_raise ArgumentError, fn ->
        Helper.resolve_directive_model(%{})
      end
    end
  end
  ...
  describe "build_directive_messages/2" do
    test "returns messages without system prompt" do
      ...
      assert Helper.build_directive_messages(messages, nil) == messages
    end

    test "prepends system prompt" do
      messages = [%{role: :user, content: "Hello"}]
      result = Helper.build_directive_messages(messages, "Be helpful")
    ...
    test "handles map with messages key" do
      ...
      result = Helper.build_directive_messages(context, nil)
  ...
  describe "add_timeout_opt/2" do
    test "adds receive_timeout when timeout provided" do
      ...
      result = Helper.add_timeout_opt(opts, 5000)
    ...
    test "returns opts unchanged when timeout is nil" do
      opts = [max_tokens: 1000]
      assert Helper.add_timeout_opt(opts, nil) == opts
    end
  end
  ...
  describe "add_tools_opt/2" do
    test "adds tools when list is not empty" do
      ...
      result = Helper.add_tools_opt(opts, tools)
    ...
    test "returns opts unchanged when tools is empty" do
      opts = [max_tokens: 1000]
      assert Helper.add_tools_opt(opts, []) == opts
    end
  end
end
