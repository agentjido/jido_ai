defmodule Jido.AI.Accuracy.IntegrationTest do
  ...
  describe "end-to-end self-consistency" do
    ...
    test "math problem: 15 * 23 = 345" do
      ...
      assert {:ok, best, metadata} = SelfConsistency.run("What is 15 * 23? Answer with just the number.",
        num_candidates: 5,
    ...
    test "simple factual question" do
      assert {:ok, best, metadata} = SelfConsistency.run(
    ...
    test "temperature variation produces diverse outputs" do
      ...
      assert {:ok, best_low, _} = SelfConsistency.run(prompt,
        num_candidates: 3,
        temperature_range: {0.0, 0.1}
      )

      # High temperature - might have some variation
      assert {:ok, best_high, _} = SelfConsistency.run(prompt,
        num_candidates: 3,
    ...
    test "all aggregators work end-to-end" do
      ...
      assert {:ok, best_mv, _} = SelfConsistency.run(prompt,
        num_candidates: 3,
        aggregator: :majority_vote
      )
      ...
      assert {:ok, best_bon, _} = SelfConsistency.run(prompt,
        num_candidates: 3,
      ...
      # Test with weighted
      assert {:ok, best_w, _} = SelfConsistency.run(prompt,
        num_candidates: 3,
        aggregator: :weighted
      )
  ...
  describe "performance and cost tracking" do
    ...
    test "token counting is accurate" do
      assert {:ok, _best, metadata} = SelfConsistency.run(
  ...
  describe "error handling" do
    ...
    test "invalid aggregator returns error" do
      assert {:error, :invalid_aggregator} = SelfConsistency.run(
        "Test",
        aggregator: String,
        num_candidates: 1
      )
    ...
    test "handles generation errors gracefully" do
      ...
      result = SelfConsistency.run(
        ...
        num_candidates: 1,
        timeout: 1  # 1ms - will likely fail
      )
  ...
  describe "chain-of-thought reasoning" do
    ...
    test "run_with_reasoning metadata is correct" do
      assert {:ok, _best, metadata} = SelfConsistency.run_with_reasoning(
