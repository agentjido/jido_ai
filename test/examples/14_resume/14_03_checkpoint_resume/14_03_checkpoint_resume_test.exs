defmodule JidoAI.Examples.CheckpointResumeTest do
  ...
  test "after-tools checkpoint preserves completed tool history and does not repeat its side effect",
    ...
    assert Enum.count(context_entries(final.context), &(&1.role == :user)) == 1
    assert_script_done(mock)
  end
  ...
  @coverage_skip_reason if(
                          :ets.whereis(:excoveralls_conf_server) != :undefined and
                            ExCoveralls.ConfServer.get() != [],
                          do: "cross-VM code fingerprints differ under cover instrumentation",
                          else: false
                        )
  @tag skip: @coverage_skip_reason
