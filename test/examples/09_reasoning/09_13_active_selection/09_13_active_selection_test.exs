defmodule JidoAI.Examples.ActiveSelectionTest do
  ...
  test "the current Adaptive selection is committed before its first provider call", %{jido: jido} do
    ...
    assert {:ok, "Unreviewed improvement"} = ActiveSelection.Agent.await(handle)
  ...
  defp with_input_control do
    definition = ActiveSelection.Agent.definition()
    ...
    attrs =
      definition |> Map.from_struct() |> Map.drop([:id, :state]) |> Map.put(:plugins, plugins)
  ...
  test "a host rejection of the selection commit stops work before the provider", %{jido: jido} do
    ...
    assert {:ok, handle} =
             ActiveSelection.Agent.ask(server, "Improve the answer", context: Map.put(context, :reject_progress, true))
  ...
  test "owner loss retains committed selection and does not replay the stopped method", %{
    ...
    {mock, context} =
      mock([%{reply: {:stream, [{:wait, :lost_selection_owner}], "stop"}}] ++ Adaptive.script(:cod))
