defmodule JidoAI.Examples.PlanningTest do
  ...
  for {method, action} <- [{:plan, Plan}, {:decompose, Decompose}, {:prioritize, Prioritize}] do
    test "#{method} retains its text and parsed result through direct calls Exec and the Plugin",
      ...
      assert {:ok, %{default_model: :planning, default_max_tokens: 4096, default_temperature: 0.7}} =
  ...
  test "configured Plugin defaults caller model and explicit model keep their precedence", %{
    ...
    assert {:ok, first} =
             Server.call(server, signal(:plan, %{goal: "Release"}), context: Map.delete(context, :model))
  ...
  test "the DSL composes Planning reasoning and ordinary routes without replacing other results",
    ...
    assert {:ok, _} =
             Server.call(
               server,
               Jido.Signal.new!("case.set", %{case_id: "release-42"}, source: "/examples/planning")
             )
    ...
    assert {:ok, reviewed} =
             Server.call(
               server,
               Jido.Signal.new!("reasoning.cot.run", %{prompt: "Review"}, source: "/examples/planning"),
               context: context
             )
  ...
  test "the request timeout closes provider work before the outer Exec deadline" do
    ...
    task =
      Task.async(fn ->
        Jido.Exec.run(Plan, %{goal: "Release", timeout: 50}, %{context | model_options: options}, timeout: 3_000)
      end)
