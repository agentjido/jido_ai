defmodule JidoAI.Examples.ChatTest do
  ...
  test "tool discovery and execution retain aliases through all three entry points", %{jido: jido} do
    ...
    assert {:ok, agent} =
             Server.call(server, Example.signal("list_tools", %{include_schema: false}), context: context)
  ...
  test "the Chat capability executes aliased tools and returns the model answer", %{jido: jido} do
    ...
    assert {:ok, agent} =
             Server.call(server, Example.signal("message", %{prompt: "Prepare the case"}), context: context)
  ...
  test "later provider failure returns an action error after completed tool work" do
    {mock, context} =
      mock([%{reply: {:tools, [Example.tool()]}}, %{reply: {:error, 503, "unavailable"}}])

    context = Map.put(context, :tools, %{"label" => Example.Echo})

    assert {:error, %Jido.Action.Error.ExecutionFailureError{} = reason} =
             CallWithTools.run(%{prompt: "Label", auto_execute: true}, context)
  ...
  test "capability binding ignores forged action and result targets", %{jido: jido} do
    ...
    assert {:error, _} =
             Server.call(bad_server, Example.signal("simple", %{prompt: "Label"}), context: context)
  ...
  test "the agent DSL combines all Chat routes with ordinary domain work", %{jido: jido} do
    ...
    assert {:ok, %{available_tools: ["label"], tools: %{"label" => Example.Echo}, auto_execute: true}} =
  ...
  test "all LLM Actions emit canonical telemetry with real decoded token counts" do
    ...
    for {kind, action, params, expected} <- [
          ...
          {:generate_object, GenerateObject, %{prompt: "Label", object_schema: Zoi.object(%{label: Zoi.string()})}, 15},
  ...
  test "a later model failure returns an error and leaves capability state unchanged", %{
    jido: jido
  } do
    {mock, context} =
      mock([%{reply: {:tools, [Example.tool()]}}, %{reply: {:error, 503, "Unavailable"}}])
