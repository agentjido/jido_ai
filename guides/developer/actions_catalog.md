...
- `Jido.AI.Actions.Skill.LoadSkill`
  ...
  - Pair with `Jido.AI.Skill.Prompt.render_registry_index/1` for tag-filtered skill indexes, or use a profile `skills` block for scoped filesystem and runtime-spec catalogs.
...
- `Jido.AI.Actions.Skill.LoadResource`
  ...
  - Required params: `name` plus exactly one selector: `resource_id` for provider-backed runtime skills, or `relative_path` for filesystem skills.
