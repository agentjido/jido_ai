...
The [Agent example](../lib/examples/09_reasoning/09_08_trm/agent.ex) and
[24 integration cases](../test/examples/09_reasoning/09_08_trm_test.exs) run
...
macro and retained namespace/Strategy helpers are now covered by [09_09](09_09_trm_api.md).
...
The final result is the answer with the highest review score. Ties retain the
earlier answer. An improvement with no review cannot replace a positive scored
answer. When all scores are zero, the existing fallback returns the current
answer. The tests expose this fallback rather than claim it was reviewed.
