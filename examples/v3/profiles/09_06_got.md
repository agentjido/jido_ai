...
The [Agent example](../lib/examples/09_reasoning/09_06_got/agent.ex) and
[22 integration cases](../test/examples/09_reasoning/09_06_got_test.exs) run the
...
| `aggregation_strategy` | `:synthesis` | Retained setting; accepts `:synthesis`, `:voting`, `:weighted`. Distinct voting/weighted execution remains an open requirement |
...
The baseline stores `aggregation_strategy` but does not use it to select voting
or weighted execution. The tests prove setting retention only. They do not
claim those algorithms are complete. Default runs also do not demonstrate
general branching or aggregation across several leaves. Those behaviors need
explicit examples and implementation before full method acceptance. Core Flow
remains the composition layer.
...
[09_07](09_07_got_api.md). Tool declarations and steering
...
[09_07 example](09_07_got_api.md) adds public GoTAgent helpers and retained
...
```sh
mix test --include integration --seed 0 test/examples/09_reasoning/09_06_got_test.exs
```
