...
The [Agents](../lib/examples/14_resume/14_11_initial_state/agent.ex) and
[tests](../test/examples/14_resume/14_11_initial_state_test.exs) define seven
integration cases. They use native Agent DSL and public AI Agents, buffered and
...
the old Context ID. Import does not recreate a prior core Thread revision or
claim ownership of old request IDs. Existing message refs stay on the messages;
new core Thread entries use current owned request identity.
...
Standalone State conversion remains the separate [14_09 API](14_09_state_migration.md).
...
```sh
mix test test/examples/14_resume/14_11_initial_state_test.exs --include integration --include pending_dsl
```
