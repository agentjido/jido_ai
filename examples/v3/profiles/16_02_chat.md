...
The [Agent example](../lib/examples/16_capabilities/16_02_chat/agent.ex) declares
the Chat Plugin and seven explicit routes beside an ordinary domain route.
The [integration tests](../test/examples/16_capabilities/16_02_chat_test.exs)
use one HTTP mock. ReqLLM, core Flow, tool Actions and Agent commits execute.
The suite has 32 integration cases. Run it with:

```sh
mix test test/examples/16_capabilities/16_02_chat_test.exs --include integration --seed 0
```
...
- Unknown or invalid tools become error tool messages. The model can recover.
  A first provider failure returns an error tuple through core execution. A
  later provider failure retains the legacy successful tuple containing
  `type: :error`, its cause, round count and completed usage. Consumers must
  inspect `type`; this is not a successful model answer.
...
Signals. The callable error-result shape and descriptive `tool_policy` need
explicit consumer guidance at cutover. Invalid structured output retains completed provider usage in Action error
