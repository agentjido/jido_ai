...
[Agent](../lib/examples/02_requests/02_22_request_inspection/agent.ex) and
[tests](../test/examples/02_requests/02_22_request_inspection_test.exs) use the
...
[02_23](02_23_context_operations.md). They are current Agent data, not copies
...
covered with a lost reply and a new Agent Server. Offline v2 Agent conversion,
all pending-work storage failures and guaranteed terminal delivery remain open.
...
Sixteen integration cases cover idle/unknown requests, held model and tool
...
completion recovery, refused completion and concurrent cancellation events.
...
```sh
mix test test/examples/02_requests/02_22_request_inspection_test.exs --include integration
```
