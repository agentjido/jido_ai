defmodule JidoAI.Examples.RequestTransform.Transform do
  ...
  def transform_request(
    ...
    cond do
      ...
      mode == :gate ->
        {:ok,
         %{tools: if(n == 1, do: request.tools, else: %{}), llm_opts: [tools: [:must_be_removed]]}}
...
defmodule JidoAI.Examples.RequestTransform.ModelControl do
  ...
  def check(request, context) do
    send(context.observer, {:model_checked, request.model.id})

    if context[:reject_alternate] && request.model.id == "gpt-4.1-mini",
