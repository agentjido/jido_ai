defmodule JidoAI.Examples.RequestInspectionTest do
  ...
  test "a refused completion remains pending in committed state and exposes the live failure", %{
    jido: jido
  } do
    alias JidoAI.Examples.Completion.Store
    ...
    {:ok, server} = Jido.start_agent(jido, Agent.new!(), persistence: adapter, restore: false)
    {:ok, request} = submit(server, context)
    assert {:error, {:completion_uncommitted, _} = failure} = Request.await(request)
    {:ok, view} = Session.snapshot(server)
    assert view.request.status == :pending and view.request.result == nil
    assert view.live.terminal_failure == failure
    assert view.details.phase == :request_failed
    assert List.last(view.details.trace.events).data.committed? == false
    refute Enum.any?(view.details.trace.events, &(&1.kind == :request_completed))
    assert_script_done(mock)
  end
