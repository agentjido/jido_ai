defmodule JidoAI.Examples.ContextOperationsTest do
  ...
  defp operations(server),
    do: Jido.Thread.filter_by_kind(state(server).thread, :ai_context_operation)
  ...
  for {module, signal, streaming?} <- [
    ...
    test "#{module} keeps owned Thread refs through tools input and reconstruction", %{jido: jido} do
      ...
      [first_entry] =
        Jido.Thread.filter_by_kind(state(server).thread, :ai_message) |> Enum.take(1)
      ...
      recorded = Jido.Thread.filter_by_kind(state(server).thread, :ai_message)
      ...
      assert Jido.Thread.filter_by_kind(state(restored).thread, :ai_message) == recorded

      assert {:ok, next} =
               Request.create_and_send(restored, "Continue", Keyword.delete(options, :extra_refs))

      assert {:ok, "Restored"} = Request.await(next)
      all = Jido.Thread.filter_by_kind(state(restored).thread, :ai_message)
  ...
  test "tool and assistant history receive the active lane in core Thread entries", %{jido: jido} do
    ...
    messages = Jido.Thread.filter_by_kind(state(server).thread, :ai_message)
  ...
  test "direct history changes remain in their lane after a later switch", %{jido: jido} do
    ...
    assert {:ok, _} =
             Server.call(
               server,
               Jido.Signal.new!("history.replace", %{entries: replacement.entries},
                 source: "/example"
               )
             )
    ...
    assert Enum.any?(
             Jido.Thread.to_list(state(server).thread),
  ...
  test "restored context state rejects malformed pending work and broken Thread records", %{
    ...
    assert {:ok, loaded} = Jido.Agent.instantiate(Agent.agent(), state: original)
    ...
    malformed = [
      ...
      %{value | thread: %{value.thread | entries: [:invalid]}},
      %{value | thread: %{value.thread | rev: -1}},
      %{
        value
        | thread: %{
            value.thread
    ...
    for invalid <- malformed do
      ...
      assert {:error, _} = Jido.Agent.instantiate(Agent.agent(), state: saved)
    end
  ...
  test "replacement waits for active tool work and follows the final assistant Thread entry", %{
    ...
    assert [:ai_message, :ai_context_operation] ==
             state(server).thread |> Jido.Thread.to_list() |> Enum.take(-2) |> Enum.map(& &1.kind)
