defmodule JidoAI.Examples.ToolCallbacksTest do
  ...
  defp source do
    {_, options} = Enum.find(Agent.agent().plugins, &(elem(&1, 0) == Jido.AI.Runtime.Plugin))
  ...
  test "callback effects are filtered before staging and transformer views retain completed output",
    ...
    assert_receive {:callback_view,
                    %{pending_tool_calls: [%{id: "effect", result: {:ok, %{n: 107}, [_]}}]},
                    config, %{state: %{count: 7}, agent_state: %{count: 7}}}
  ...
  test "the result callback observes a final canonical tool error once after retries", %{
    ...
    assert {:ok, request} =
             PublicAgent.ask(server, "Work",
               context: Map.merge(context, %{counter: counter(), fail_always: true})
             )
  ...
  test "DSL data Builder and source JSON execute the same explicit callback module", %{jido: jido} do
    ...
    for {definition, n} <- Enum.with_index([Agent.agent(), definition, built, decoded], 1) do
