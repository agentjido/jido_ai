defmodule JidoAI.Examples.StreamActivityTest do
  ...
  defp source do
    {_, options} =
      Enum.find(NativeAgent.agent().plugins, &(elem(&1, 0) == Jido.AI.Runtime.Plugin))
  ...
  test "DSL data Builder and source JSON execute the same idle and heartbeat policy", %{
    ...
    assert definition == NativeAgent.agent()
    ...
    for definition <- [NativeAgent.agent(), definition, built, decoded] do
