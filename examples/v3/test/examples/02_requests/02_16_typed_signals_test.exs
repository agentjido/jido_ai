defmodule JidoAI.Examples.TypedSignalsTest do
  ...
  @definitions [
    ...
    {Signal.RequestCompleted, %{request_id: "request", result: nil}, "ai.request.completed",
     "/ai/request"},
    {Signal.RequestError, %{request_id: "request", reason: :busy, message: "Busy"},
     "ai.request.error", "/ai/strategy"},
    {Signal.RequestFailed, %{request_id: "request", error: nil}, "ai.request.failed",
     "/ai/request"},
    {Signal.RequestStarted, %{request_id: "request", query: "Help"}, "ai.request.started",
     "/ai/request"},
    {Signal.ToolResult, %{call_id: "tool", tool_name: "echo", result: nil}, "ai.tool.result",
     "/ai/tool"},
    {Signal.ToolStarted, %{call_id: "tool", tool_name: "echo"}, "ai.tool.started", "/ai/tool"},
    {Signal.Usage, %{call_id: "model", model: "fixture", input_tokens: 2, output_tokens: 1},
     "ai.usage", "/ai/usage"}
  ]
  ...
  test "live early tool activity and both model rounds project through core delivery with stable correlation",
    ...
    assert_receive {:jido_ai_request_event,
                    %{kind: :llm_delta, data: %{chunk_type: :tool_call}} = early},
                   1_000
  ...
  test "the real model response helper and embedding payload retain provider data in delivered Signals",
    ...
    assert_receive {:signal,
                    %{type: "ai.embed.result", data: %{result: {:ok, [[0.1, 0.2], [0.3, 0.4]]}}}}
  ...
  test "DSL data Builder and source JSON produce the same projected model and completion Signals",
       %{jido: jido} do
    {_, options} = Enum.find(Chat.agent().plugins, &(elem(&1, 0) == Jido.AI.Runtime.Plugin))
    ...
    attrs = data |> Jido.Agent.to_map() |> Map.drop([:id, :state])
    ...
    assert Chat.agent() == data and data == built and built == decoded
    {mock, context} = mock(List.duplicate(%{reply: {:text, ["Same answer"]}}, 4))

    for definition <- [Chat.agent(), data, built, decoded] do
