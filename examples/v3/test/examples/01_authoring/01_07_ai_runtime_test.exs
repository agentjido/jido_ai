defmodule JidoAI.Examples.AIRuntimeTest do
  ...
  defp base do
    Jido.Agent.new!(
      ...
      schema: AIRuntime.Agent.agent().schema,
  ...
  defp profile do
    {_module, options} = List.last(AIRuntime.Agent.agent().plugins)
  ...
  test "a host can assign stable IDs to the AI boundary and execute a stored Agent", %{
    ...
    {_, options} = List.last(definition.plugins)
  ...
  test "route defaults survive lowering and conflict with duplicate bindings" do
    ...
    route =
      Jido.Agent.Authoring.route("ai.ask", Authoring.ai(:assistant),
  ...
  defp source_registry(profile) do
    ...
    entries = Map.new(atoms, &{"atoms/#{&1}", {:atom, &1}})
