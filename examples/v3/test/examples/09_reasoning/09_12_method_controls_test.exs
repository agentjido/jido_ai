defmodule JidoAI.Examples.MethodControlsTest do
  ...
  test "public Adaptive keeps ten ReAct calls while a later TRM request can use fifteen", %{
    ...
    assert {:ok, "Improvement 4"} = AdaptiveAPI.Public.await(next)
  ...
  test "DSL data Builder source JSON direct Flow and ordinary turns resolve the same method defaults",
    ...
    assert definition == MethodControls.Agent.agent()
    ...
    for value <- [MethodControls.Agent.agent(), definition, built, decoded] do
    ...
    assert {:ok, %{result: "Improvement 4", meta: meta}} =
    ...
    assert agent.state.reply == "Improvement 4"
    assert_script_done(mock)
  end
