defmodule JidoAI.Examples.TRMTest do
  ...
  test "one review cycle selects the scored answer and keeps all three model phases", %{
    jido: jido
  } do
    ...
    assert Server.agent(server).state.reply == "First answer"
    assert_receive {:trm_checked, "First answer"}
  ...
  test "a later review scores the prior improvement and preserves feedback and bounded trace", %{
    ...
    assert {:ok, handle} = request(server, context)
    assert {:ok, "Best reviewed answer"} = Request.await(handle)
    state = machine(server, handle)
  ...
  test "convergence stops below the threshold and does not select a lower scored answer", %{
    jido: jido
  } do
    script =
    ...
    server = start(jido)
    assert {:ok, handle} = request(server, context)
    assert {:ok, "Best"} = Request.await(handle)
    state = machine(server, handle)
  ...
  for {phase, completed} <- [reasoning: 0, supervision: 1, improvement: 2] do
    test "#{phase} failure keeps the raw cause and usage and permits a later request", %{
      ...
      assert {:ok, "First answer"} = Request.await(next)
      assert_script_done(mock)
    end
  end
  ...
  test "five review cycles fit explicit common budgets and select the fourth improvement", %{
    jido: jido
  } do
    script =
    ...
    server = start(jido)
    assert {:ok, handle} = request(server, context)
    assert {:ok, "Improvement 4"} = Request.await(handle)
  ...
  test "near maximum quality keeps the existing ACT stop below an explicit threshold of one", %{
    ...
    assert {:ok, handle} = request(server, context)
    assert {:ok, "High quality"} = Request.await(handle)
  ...
  test "a request transformer cannot enable TRM tools and unsolicited tool work fails", %{
    ...
    server =
      start(jido, %{
        reasoning:
          Map.put(TRM.source().reasoning, :request_transformer, JidoAI.Examples.ToT.Transform)
      })
    ...
    Enum.reduce(
      ...
      fn {phase, text}, machine ->
        {next, _} =
          Machine.update(
            machine,
            {phase, machine.current_call_id,
             {:ok, %{text: text, usage: %{input_tokens: 2, output_tokens: 1}}}}
          )
  ...
  test "DSL data Builder source JSON direct Flow and ordinary turns use the same recursive contract",
    ...
    assert definition == TRM.Agent.agent()
    ...
    for value <- [TRM.Agent.agent(), definition, built, decoded] do
    ...
    assert {:ok, %{result: "First answer", meta: meta}} =
    ...
    assert agent.state.reply == "First answer"
    assert_script_done(mock)
  end
