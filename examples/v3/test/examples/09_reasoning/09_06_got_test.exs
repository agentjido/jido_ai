defmodule JidoAI.Examples.GoTTest do
  ...
  for mode <- [:synthesis, :voting, :weighted] do
  ...
  test "the retained Machine still emits its legacy lifecycle telemetry" do
    ...
    {completed, _} =
      Machine.update(
        machine,
        {:llm_result, "legacy",
         {:ok, %{text: "Done", usage: %{input_tokens: 2, output_tokens: 1}}}}
      )
  ...
  test "a transformer cannot enable GoT tools and unsolicited tool work fails", %{jido: jido} do
    ...
    server =
      start(jido, %{
        reasoning:
          Map.put(GoT.source().reasoning, :request_transformer, JidoAI.Examples.ToT.Transform)
      })
  ...
  test "DSL data Builder source JSON direct Flow and ordinary turns use the same graph contract",
    ...
    assert definition == GoT.Agent.agent()
    ...
    for value <- [GoT.Agent.agent(), definition, built, decoded] do
