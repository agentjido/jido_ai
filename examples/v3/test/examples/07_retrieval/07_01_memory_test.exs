defmodule JidoAI.Examples.RetrievalTest do
  ...
  test "stored memory reaches an actual Chat tool-loop request", %{jido: jido} do
    ...
    assert {:ok, _} =
             Server.call(server, Example.signal("chat.message", %{prompt: "Tokyo weather"}),
               context: context
             )
  ...
  test "configured limits bound snippets and request opt out keeps routes available", %{
    ...
    assert {:ok, _} =
             Server.call(disabled_server, Example.signal("chat.message", %{prompt: "Tokyo rain"}),
               context: context
             )
  ...
  test "empty stores and non-enriched Chat routes retain the original request", %{jido: jido} do
    ...
    assert {:ok, _} =
             Server.call(server, Example.signal("chat.message", %{prompt: "Tokyo rain"}),
               context: context
             )
    ...
    assert {:ok, _} =
             Server.call(server, Example.signal("chat.simple", %{prompt: "Tokyo rain"}),
               context: context
             )
  ...
  test "store absence fails before model work and opt out still permits a request", %{jido: jido} do
    ...
    assert {:error, _} =
             Server.call(server, Example.signal("case.review", %{query: "Tokyo"}),
               context: context
             )
  ...
  test "the Agent DSL reads inserted memory during native AI execution", %{jido: jido} do
    ...
    assert {:ok, agent} =
             Server.call(server, Example.signal("case.review", %{query: "Tokyo weather"}),
               context: context
             )
  ...
  test "pure command preparation does not read an external store", %{jido: _jido} do
    ...
    assert {:ok, candidate, []} =
             Jido.Agent.cmd(agent, Example.signal("case.review", %{query: "Tokyo weather"}),
               context: context
             )
  ...
  test "a model calls the real memory Action and receives its stored result", %{jido: jido} do
    {mock, context} =
      mock([
        %{
          reply:
            {:tools, [%{id: "memory-call", name: "recall_memory", arguments: %{query: "Tokyo"}}]}
        },
