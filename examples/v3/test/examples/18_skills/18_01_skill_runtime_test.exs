defmodule JidoAI.Examples.SkillRuntimeTest do
  ...
  defp bind_catalog(context, opts \\ []) do
    integration =
      AgentIntegration.prepare!(
        Keyword.merge([specs: [spec()], resource_provider: {Provider, :handle}], opts)
      )
  ...
  defp compact(server),
    do:
      Session.modify_context(server, %{
        ...
        reason: :compaction,
        result_context:
          Context.new(system_prompt: "After compaction") |> Context.append_user("Summary")
      })
  ...
  test "a real activation survives compaction and reaches the next HTTP request", %{jido: jido} do
    ...
    assert Jido.Thread.filter_by_kind(
             Server.agent(server).state.jido_ai_contexts.assistant.thread,
             :ai_message
           )
  ...
  test "different Flow workers share activation and fresh resource loads across requests", %{
    ...
    assert {:ok, "Done"} =
             Agent.ask_sync(server, "Refresh",
               context: Map.put(context, :resource_content, "Fresh")
             )
  ...
  test "a new trusted host binding does not reuse an old same-name activation", %{jido: jido} do
    ...
    assert {:ok, "Done"} =
             Agent.ask_sync(server, "Second",
               context: bind_catalog(context, specs: [spec("New instructions")])
             )
  ...
  test "two Agents isolate same-name activations and owner loss clears only its sessions", %{
    ...
    assert {:ok, "Done"} =
             Agent.ask_sync(second, "Second",
               context: bind_catalog(context, specs: [spec("Second Agent")])
             )
  ...
  for {kind, content, metadata} <- [
        {:image, <<137, "PNG", 13, 10, 26, 10, 0, 255>>,
         %{filename: "plot.png", mime_type: "image/png"}},
