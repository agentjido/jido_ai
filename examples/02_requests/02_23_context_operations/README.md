...
[Agents](agent.ex) and
[tests](../../../test/examples/02_requests/02_23_context_operations/02_23_context_operations_test.exs) use core
...
## State, Session, And Thread Ownership

`jido_ai_contexts[profile_id]` contains the active ref, pending operation,
applied IDs and a portable `Jido.Session` value under `:session`. The Session
owns one append-only `Jido.Thread`. Request admission and history
...
user operation ID. There is one session thread per profile and no second message loop.

The session thread can grow with conversation history. The Agent's state-size limit
...
operation shape, unique bounded IDs, log sequence/count data and message
...
proof in [18_01](../../18_skills/18_01_skill_runtime/README.md). User refs cannot establish runtime
skill provenance. Direct host imports remain a trusted operation.

## Evidence and limits

Thirty-one example cases cover idle/deferred operations, lane isolation,
...
No old Strategy callback or `Jido.Thread.Agent` API was restored. Full old Agent
...
```sh
mix test test/examples/02_requests/02_23_context_operations/02_23_context_operations_test.exs --include example
```
...
log entry refs keep the owned request and run IDs. Caller `signal_id`,
`request_id` and `run_id` values cannot replace log identity, including string
