...
through the native catalog's fallback.
...
[14_03](../14_03_checkpoint_resume/README.md) proves native model and
complete-tool-round checkpoints. Query append on resume and an external
pending-input queue are refused before provider work. Partial tool batches,
durable replay control, and Agent persistence are outside this example. No Exec
value goes into a token.
...
The full package uses the local V3 dependencies.

## Validation

The example cases cover lazy start; identity; real aliased tools; usage;
terminal replay and cancellation; stream deltas; consumer and Agent
...
Standalone callers can supply `context.agent_module` with before and after tool
callbacks. The
adapter binds those callbacks to the shared native profile interceptor.
Tests prove the exact Action input and model tool-result JSON for before-only,
after-only, and combined callbacks.
